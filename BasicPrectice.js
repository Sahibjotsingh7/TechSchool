  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await fetch('http://localhost:8080/api/mcqs/basemcqs', {
          headers: {
  ...
  return (
    <div className="container">
      ...
      <div className="header">
        <h1 style={{ marginTop: "100px" }}>Basic Concepts Quiz</h1>
      </div>

      {showResultsbutton ? (
        <div>
          <button className="submit-button" onClick={() => { setShowResults(true); }}>View Results</button>
        </div>
      ) : (
